...
public class StructureGenerator : MonoBehaviour {
    ...
    public void GenerateSubstructures(int[,] map, GameObject structureInstance, int posX, int posY, string seed)
    {
        int[,] localMap = new int[structureSize, structureSize];
        ...
        for(int x = posX - structureSize / 2; x < map.Length && ((structureSize % 2 == 0)? (x < posX + structureSize / 2) : (x < posX + structureSize / 2 + 1)); x++) {
            ...
            {
                localMap [x % structureSize, y % structureSize] = map [x,y];
            }
        }
        ...
        for (int k = 0; k < objectsToSpawn.Count; k++)
        {
            ...
            if (objectToSpawn != null)
            {
                ...
                propPlacer.ObjectMapGenerator(localMap, 1, seed + k, objectSpawnChance[k], objectToSpawn);
            }
        }
    }
}
