...
public class MeshGenerator : MonoBehaviour {
	...
	HashSet<int> checkedVertices = new HashSet<int>();	//	We use a hashset here because .Contains checks will work faster on a HashSet :/
	...
	void AssignVertices (Node[] points){
		for (int i = 0; i<points.Length; i++) {
			if (points [i].vertexIndex == -1) {	// value by default
	...
	int GetConnectedOutlineVertex(int vertexIndex) {
		...
		for (int i = 0; i < trianglesContainingVertex.Count; i ++) {
			...
			for (int j = 0; j < 3; j ++){
				int vertexB = triangle[j];	// This is possible thanks to the code in triangle (the public int this[...] bit)!
	...
	void OnDrawGizmos ()	// Unecessary, was made for testing purposes!
	{
		/*
		if (squareGrid != null) {
			for (int x = 0; x<squareGrid.squares.GetLength(0); x++) {
			...
			for (int x = 0; x<nodeCountX; x++) {
				for (int y = 0; y<nodeCountY; y++) {
					Vector3 position = new Vector3 (
						-mapWidth / 2 + x * squareSize + squareSize / 2, 0,
